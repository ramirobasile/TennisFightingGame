    ...
    public abstract class Camera
    {
        ...
        public virtual void Update()
		{
			...
			if (shake > 0)
			{
				...
				offset.X = MathHelper.Lerp(offset.X, goal.X, ShakeSpeed * Game1.DeltaTime);
				offset.Y = MathHelper.Lerp(offset.Y, goal.Y, ShakeSpeed * Game1.DeltaTime);
			}
			else
			{
				offset = Vector2.Zero;
			...
			transform =
                ...
                Matrix.CreateTranslation(new Vector3(
                    (-centre.X + offset.X) * zoom + Game1.Viewport.Width / 2,
                    (-centre.Y + offset.Y) * zoom + Game1.Viewport.Height / 2, 0)
                );
        }
    }
}