...
namespace TennisFightingGame
{
    ...
    public static class Helpers
	{
			...
			float pan = 0, bool loops = false)
		{
			...
			if (!Game1.ConfigFile.Boolean("Sound", "Mute"))
			{
				instance.Volume = volume * Game1.ConfigFile.Number("Sound", "SFXVolume") / 100;
			}
			else
			{
				instance.Volume = 0;
			...
			if (!Game1.ConfigFile.Boolean("Sound", "Mono"))
			{
				instance.Pan = pan;
			}
			else
			...
			float pitch = 1, float pan = 0)
		{
			...
			int index = Game1.Random.Next(0, sounds.Length);
			...
			float pitch = 1, float pan = 0)
		{
			...
			{
				int index = Game1.Random.Next(0, sounds[i].Length);
